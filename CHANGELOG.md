...
- 反代：新建入口时校验监听端口占用，避免重复端口 silently 冲突

### 改进

- 反代：入口/规则「复制」改为打开表单预填原内容，由用户确认后再保存（不再自动创建、不改域名后缀）
- 反代：入口分组子表布局撑满卡片宽度；域名后紧跟复制/打开链接；状态列仅保留开关
- 反代：同端口监听配置冲突提示仅在配置确实不一致时显示
- FRP：TCP 隧道远程端口手动指定支持 1–65535（0 仍为自动分配，随机范围 1024–65535）
- 通知：各通道密钥解密失败不再拖垮其它通道；事件开关保存与前端状态同步更可靠

### 修复

- 通知：旧 SMTP 等密文无法解密时，Telegram/Webhook 等已配置通道仍无法告警的问题
